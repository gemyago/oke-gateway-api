...
func TestOciLoadBalancerModelImpl(t *testing.T) {
	...
	t.Run("reconcileBackend", func(t *testing.T) {
		t.Run("create new backend", func(t *testing.T) {
			...
			params := reconcileBackendParams{
				...
				backend:        makeRandomOCIBackendDetails(),
			}
			...
			ociLoadBalancerClient.EXPECT().CreateBackend(t.Context(), loadbalancer.CreateBackendRequest{
			...
			wantUpdatedBs := makeRandomOCIBackendSet()
			ociLoadBalancerClient.EXPECT().GetBackendSet(t.Context(), loadbalancer.GetBackendSetRequest{
				BackendSetName: params.backendSet.Name,
				LoadBalancerId: &params.loadBalancerID,
			}).Return(loadbalancer.GetBackendSetResponse{
				BackendSet: wantUpdatedBs,
			}, nil)

			actualBackendSet, err := model.reconcileBackend(t.Context(), params)
			require.NoError(t, err)
			assert.Equal(t, wantUpdatedBs, actualBackendSet)
		})
		...
		t.Run("backend already exists", func(t *testing.T) {
			...
			existingBackendDetails := makeRandomOCIBackendDetails()
			existingBackend := loadbalancer.Backend{
				IpAddress: existingBackendDetails.IpAddress,
				Port:      existingBackendDetails.Port,
			}
			...
			params := reconcileBackendParams{
				...
				backendSet: loadbalancer.BackendSet{
					...
					Backends: []loadbalancer.Backend{
						makeRandomOCIBackend(),
						existingBackend,
						makeRandomOCIBackend(),
					},
				},
				backend: existingBackendDetails,
			}

			actualBackendSet, err := model.reconcileBackend(t.Context(), params)
			require.NoError(t, err)
			assert.Equal(t, params.backendSet, actualBackendSet)
		})
	})
}
